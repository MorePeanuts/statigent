...
def _render_content(content: str, collapse: int) -> Markdown | Syntax:
...
def _panel_title(message: dict[str, Any], index: int) -> str:
    ...
    return f"#{index} {agent} s{session} - {name} ({role})"


def _render_message(message: dict[str, Any], index: int, collapse: int) -> None:
    agent = str(message.get("agent") or "unknown")
    ...
    if content:
        console.print(
            Panel(
                _render_content(content, collapse),
...
def main(
    ...
    with console.pager(styles=True):
        ...
        for index, message in enumerate(messages, start=1):
            _render_message(message, index, collapse)
