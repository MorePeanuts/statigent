...
DEA_ACTION_PROMPTS: dict[ExplorationActionKind, str] = {
    ExplorationActionKind.INSPECT_SCHEMA: (
        "Inspect table shapes, columns, dtypes, identifiers, and possible join keys."
    ),
    ExplorationActionKind.PROFILE_MISSINGNESS: (
        "Profile missing values by table and column, including rates and patterns."
    ),
    ExplorationActionKind.SUMMARIZE_NUMERIC: (
        "Summarize numeric columns with distribution statistics and notable extremes."
    ),
    ExplorationActionKind.SUMMARIZE_CATEGORICAL: (
        "Summarize categorical columns with cardinality, frequent values, "
        "and imbalance."
    ),
    ExplorationActionKind.ANALYZE_TIME_TREND: (
        "Analyze time-indexed patterns, trends, seasonality, and temporal gaps."
    ),
    ExplorationActionKind.ANALYZE_GROUP_COMPARISON: (
        "Compare metrics across meaningful groups and report material differences."
    ),
    ExplorationActionKind.ANALYZE_CORRELATION: (
        "Estimate relationships among variables and note correlation limitations."
    ),
    ExplorationActionKind.DETECT_OUTLIERS: (
        "Detect unusual rows or values and explain their likely impact on conclusions."
    ),
    ExplorationActionKind.VALIDATE_DATA_QUALITY: (
        "Validate consistency, duplicates, ranges, keys, and other data quality risks."
    ),
    ExplorationActionKind.CREATE_VISUALIZATION: (
        "Create a focused visualization that directly tests the approved question."
    ),
    ExplorationActionKind.ANSWER_SPECIFIC_QUESTION: (
        "Compute evidence that directly answers a specific user or reviewer question."
    ),
    ExplorationActionKind.CUSTOM_ANALYSIS: (
        "Run a constrained custom analysis with explicit rationale, evidence, and risk."
    ),
}
