"""Evaluate Data Interpreter baseline agent on DSBench data modeling tasks."""
...
from statigent.baseline import DataInterpreterBaselineAgent
...
def _load_registry_path(registry_path: Path | None, console: Console) -> str | None:
...
def main(
    ...
    task_id: Annotated[
        str | None,
        typer.Option(help="Run a specific competition by name (e.g. titanic)."),
    ] = None,
    ...
    path = _load_registry_path(registry_path, console)
    ...
    console.print("[bold]DSBench Data Interpreter Data Modeling Evaluation[/bold]")
    ...
    agent = DataInterpreterBaselineAgent(model_name=model)

    console.print("\n[blue]Preparing DSBench data modeling data...[/blue]")
    adapter.prepare()
    console.print(f"  {len(adapter._samples)} competitions available")
...
def _print_done(console: Console, result: Any) -> None:
    total = result.others.get("total_competitions", "?")
    completed = sum(
        1
        for competition in result.details.get("per_competition", [])
        if competition.get("raw_score") is not None
    )
    console.print(
        f"\n[bold green]Done — evaluated {completed}/{total} competitions[/bold green]"
    )
