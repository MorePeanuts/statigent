"""Evaluate Data Interpreter baseline agent on DSBench data analysis tasks."""
...
def main(
    ...
    console.print("[bold]DSBench Data Interpreter Data Analysis Evaluation[/bold]")
    ...
    adapter = DSBenchAdapter(task="data_analysis", judge_model_name=judge_model)
    agent = DataInterpreterBaselineAgent(model_name=model)

    console.print("\n[blue]Preparing DSBench data analysis data...[/blue]")
    adapter.prepare()
    total_questions = sum(len(s.get("questions", [])) for s in adapter._samples)
    console.print(f"  {len(adapter._samples)} samples ({total_questions} questions)")
    ...
    if resume_dir is not None:
        ...
        console.print(
            "\n[bold green]Done — evaluated "
            f"{result.total_tasks} questions[/bold green]"
        )
...
def _resume_run(
    ...
    console.print(
        f"\n[bold green]Done — evaluated {result.total_tasks} questions[/bold green]"
    )
